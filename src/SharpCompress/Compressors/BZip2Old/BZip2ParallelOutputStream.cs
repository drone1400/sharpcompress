    ...
    public class BZip2ParallelOutputStream : Stream
    {
        ...
        private bool TryCreateNewProcessingThread()
        {
            lock (this._syncRootActiveThread)
            {
                ...
                {
                    Thread thread = new Thread(this.MultiThreadWorkerAction) { Name = $"PBZip2 - Thread #{this._mtNextThreadId++}", IsBackground = true, Priority = ThreadPriority.Normal, };
        ...
        private bool TryWriteOutputBlockAndIncrementId()
        {
            ...
            try
            {
                ...
                // update file CRC
                this._streamCrc = ((this._streamCrc << 1) | (this._streamCrc >> 31)) ^ (uint)currentOutput.BlockCrc;
        ...
        private void WriteBz2Header()
        {
            ...
            this._bitStream.WriteBits(8, 0x30 + this._blockLevel); // block level digit
        }
        ...
        private void WriteBz2FooterAndFlush()
        {
            ...
            // write combined CRC
            this._bitStream.WriteBits(16, (int)((this._streamCrc >> 16) & 0xFFFF));
            this._bitStream.WriteBits(16, (int)(this._streamCrc & 0xFFFF));
